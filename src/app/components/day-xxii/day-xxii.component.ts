...
@Component({
  selector: 'app-day-xxii',
  templateUrl: './day-xxii.component.html',
  styleUrls: ['./day-xxii.component.scss']
})
export class DayXXIIComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.httpClient.get('assets/input-data/input22.txt', { responseType: 'text' }).subscribe(data => {
      this.parseInput(data);
      this.result1 = this.walk();

      this.current = [Infinity, Infinity, 'R'];
      this.parseInput(data);
      this.result2 = this.walk(true);
    });
  }
