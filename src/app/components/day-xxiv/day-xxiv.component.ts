...
@Component({
  selector: 'app-day-xxiv',
  templateUrl: './day-xxiv.component.html',
  styleUrls: ['./day-xxiv.component.scss']
})
export class DayXXIVComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.httpClient.get('assets/input-data/input24.txt', { responseType: 'text' }).subscribe(data => {
      this.parseInput(data);
      this.result1 = this.travelTime(this.start, this.end, 0);
      const backForSnacks = this.travelTime(this.end, this.start, this.result1);
      this.result2 = this.travelTime(this.start, this.end, backForSnacks);
    });
  }
